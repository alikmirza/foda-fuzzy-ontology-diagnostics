"""Standalone evaluation harness for yRCA against RCAEval.

Mirrors ``evaluate_microrca.py`` with one yRCA-specific diagnostic
column and an explicit **offset-robustness** decomposition that the
brief introduces as a standard reported axis (Paper 6 §4):

* ``AC@1_random`` — yRCA's :func:`detect_onset` is replaced by a
  uniformly-random in-band pivot. Detector-vs-rule-engine
  decomposition: does yRCA's value live in the onset detector, or
  in the rule-engine reasoning over the synthesised events?

* ``AC@1_a_standard`` / ``AC@1_b_edges_mean`` / ``AC@1_c_centered``
  — re-normalize the case with the inject offset placed at the
  per-case hashed default (standard), at the two band extremes
  ``5 %`` / ``95 %`` (edges), and at the band centre ``50 %``
  (centered). This decomposes the offset-robustness axis: how much
  AC@1 the method retains when injection sits near the window
  edges vs. comfortably inside the central [25 %, 75 %] search
  band. See ``paper/notes/findings.md`` → "Universal edge
  fragility" for the cross-method observation.
...
* **shift_minus** / **shift_plus** — re-normalize with the offset
  ...
  construction yRCA doesn't read ``ground_truth``, so the shifted
  AC@1 must equal the true AC@1 — the structural witness of
  ``S(yRCA) = 0``.
...
Usage::
    ...
    python -m evaluation.experiments.evaluate_yrca \\
        --data ~/datasets/rcaeval/RE1/RE1-OB \\
        --out results/week2_yrca_validation.csv \\
...
from ..methods import yrca as yrca_mod
...
# Offset-robustness regime offsets (as a fraction of window_seconds).
# ``edge_left`` and ``edge_right`` are the two band extremes; their
# mean is reported as ``AC@1_b_edges_mean`` to match the cross-method
# diagnostic CSV column shape (results/cross_method_offset_diagnostic.csv).
...
def evaluate(
    ...
    """Run yRCA on every RCAEval case under ``data_path``.
...
def _maybe_shifted_ac1(
    ...
    Only ``ground_truth`` shifts; ``case_window`` is identical. yRCA
...
def _random_onset_ac1(
    ...
    """Re-score with yRCA's :func:`detect_onset` replaced by a
    uniformly-random in-band pivot. Detector-vs-rule-engine
    decomposition.
    """
    ...
    original = yrca_mod.detect_onset

    def _fake_detect_onset(case_window, services, **kwargs):
        return pivot

    yrca_mod.detect_onset = _fake_detect_onset
    try:
        out = method.diagnose_normalized(norm_true)
    finally:
        yrca_mod.detect_onset = original
    return float(accuracy_at_k(out.ranked_list, ground_truth, 1))
...
def _offset_robustness_ac1(
    ...
    Returns a dict with five AC@1 columns:

    * ``AC@1_a_standard`` — the per-case SHA-derived hashed offset.
    * ``AC@1_b_edge_left`` — offset = ``5 %`` of window_seconds.
    * ``AC@1_b_edge_right`` — offset = ``95 %`` of window_seconds.
    * ``AC@1_b_edges_mean`` — mean of the two edges (compatible
      with the column shape of the cross-method offset diagnostic
      CSV).
    * ``AC@1_c_centered`` — offset = ``50 %`` of window_seconds.

    Each AC@1 is ``nan`` when normalization fails for the regime
    (e.g. raw telemetry doesn't cover the requested span).
    """
